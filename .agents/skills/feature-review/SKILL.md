...
## Adversarial Mode (always on — default)

Reviews are adversarial by default. **Never trust a claim you can verify.** The tracker, memory, tests, and the implementer's own summary are *hypotheses*, not evidence.

Rules:

1. **Verify claims by execution.** "Tests pass", "smoke worked", "CORS is configured" — re-run, re-probe, re-curl. A claim that was true yesterday can be broken by today's commit.
2. **Hunt for the failure, don't wait for it.** Actively probe the unhappy paths: bad input, missing config, expired data, empty states, duplicate submissions, dead network. A feature that only works on the happy path is a finding, not a pass.
3. **Distrust your own tooling.** Display/redaction layers, caches (turbo, HTTP), and stale builds can fake a pass. Verify on-disk state and live behavior with non-printing probes where output may be filtered.
4. **Attack the seams.** Most real bugs live between components: cookie semantics vs. browser context (e.g. SameSite inside WebViews), cross-origin boundaries, async timing in tests, env vars that are set-but-empty, race conditions on upserts.
5. **Report suspicions honestly with evidence grades.** Label findings as `verified` (reproduced live), `likely` (strong static evidence), or `suspected` (needs a check that wasn't possible). Never present a suspicion as a confirmed bug, and never bury a verified one as a nitpick.

Being adversarial is about evidence, not hostility toward the code: the goal is to find what is *wrong*, prove it, and hand the developer a decision — not to invent problems.
