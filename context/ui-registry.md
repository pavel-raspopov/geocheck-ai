...
| `canvas-view`    | shipped | рендер чертежа + оверлей линий/меток/вершин | `src/ui/canvas-view.ts`; демо (Фаза 1) и реальный пайплайн (Фаза 4: contain-fit + `CanvasOverlay` через `drawOverlay`); цвета из CSS-переменных                      |
| `verdict-card`   | shipped | вердикт + отклонение, `aria-live`           | `src/ui/verdict-card.ts` `createVerdictCard` / `updateVerdictCard(card, verdict, notes?)` + `setVerdictPlaceholder`; софт-ноты из `unboundLabels` (`.verdict-notes`) |
...
| `image-input`    | shipped | File → RawImage (DOM-адаптер анализа)       | `src/ui/image-input.ts` `fileToRawImage(file)`; `createImageBitmap` + canvas; проверяется QA-харнессом                                                               |
