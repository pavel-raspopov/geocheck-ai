...
- **Endpoint (browser `fetch`, no SDK):** `POST https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent`, auth header **`x-goog-api-key`** (chose header over `?key=` query param — keeps the key out of URLs/logs). Verified against official docs (ai.google.dev/api/generate-content): body `{ contents: [{ parts: [{ text }] }], generationConfig: { responseMimeType: 'application/json', temperature: 0 } }`; response text at `candidates[0].content.parts[*].text` (parts may be multiple — join them). **Model updated to `gemini-3.6-flash` 2026-09-18 after the live CORS check returned 404 («use models/gemini-3.6-flash») for `gemini-2.0-flash` — the 404 itself confirms CORS/network work; no hang observed, so no AbortController for now.**
...
- **CORS (feature 11, 2026-09-17):** still **not verified end-to-end** — needs a real user key in the user's browser. Probe result from the dev sandbox (`scripts/qa/gemini-cors-probe.mjs`, invalid key): the `fetch` neither resolved nor rejected within 60 s — no response, no CORS console error (sandbox egress to Google appears blocked; a hanging fetch leaves the fallback button busy — known limitation, no AbortController timeout in scope). Re-run the probe with a real key on an unrestricted network: type it into the UI field, click «Уточнить через ИИ», check `GEMINI_RESPONSES` status (4xx ⇒ CORS fine, bad key) vs missing response/CORS console error ⇒ blocked.
