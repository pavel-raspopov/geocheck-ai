...
- **Текст → правила** — оффлайн-парсер распознаёт правила из текста (основной путь, покрыт юнит-тестами); при ошибке парсера — ручной фолбэк через Gemini (ключ вводится в UI-поле и живёт только в localStorage, в репозитории ключей нет)
